...
class MICROPolicy(BasePolicy):
    """
    Model-Based Offline Reinforcement Learning with a Conservative Bellman Operator
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        obss, actions, next_obss, rewards, terminals = mix_batch["observations"], mix_batch["actions"], mix_batch[
            "next_observations"], mix_batch["rewards"], mix_batch["terminals"]
        batch_size = obss.shape[0]

        # update critic
        qs = torch.stack([critic(obss, actions) for critic in self.critics], 0)
        with torch.no_grad():
            uncertainty = self.state_uncertainty(obss, actions)
            uncertainty[:len(real_batch["rewards"])] = 0.0
            ...
            if self._max_q_backup:
                ...
                tmp_next_qs = torch.cat(
                    [critic_old(tmp_next_obss, tmp_next_actions) for critic_old in self.critics_old], 1)
                tmp_next_qs = tmp_next_qs.view(batch_size, 10, len(self.critics_old)).max(1)[0].view(-1,
                                                                                                     len(self.critics_old))
            ...
            target_q = rewards + self._gamma * (1 - terminals) * (next_q - self._penalty_coef * uncertainty)
    ...
    def q_fun(self, obs, act):
        next_qs = torch.cat([critic_old(obs, act) for critic_old in self.critics], 1)
        next_q = torch.min(next_qs, 1)[0].reshape(1, -1)
        return next_q
    ...
    def state_uncertainty(
        ...
        next_qs = pred_next_qs.mean(0)
        uncertainty = next_qs.mean(0) - next_qs.min(0).values
